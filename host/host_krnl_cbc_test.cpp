...
#define krnl_cbc_arg_WORDS_NUM  2
#define krnl_cbc_arg_SRC_ADDR   3
#define krnl_cbc_arg_DEST_ADDR  4
...
void add_one_data (const unsigned char* input, unsigned char* output, int words, int groups, int mode )
{
    unsigned char *in_data, *out_data;
    unsigned char carry;
    for (int g = 0; g < groups; g++) {
        for (int w = 0; w < words; w++) {
            in_data = (unsigned char*)((unsigned long long)input + words * 64 * g + w * 64);
            out_data = (unsigned char*)((unsigned long long)output + words * 64 * g + w * 64);
            for(int j=0; j < 4; j++){
                if (mode) {
                    //AES_ecb_encrypt(in_data, out_data, &aes_key, AES_ENCRYPT);
                    carry = 1;
                    for(int i = 15; i >= 0; i--){
                        if(carry == 1) {
                            out_data[j*16 + i] = in_data[j*16 + i] + 1;
                            carry = 0;
                            if(out_data[j*16 + i] == 0) carry = 1;
                        }else{
                            out_data[j*16 + i] = in_data[j*16 + i];
                        }
                    }
                }
            }
...
void execute_cipher_thread(xrt::run run_krnl_cbc, xrt::bo input_buffer, xrt::bo output_buffer, int words_num, int mode, int cbc_mode) {
                        ...
                        int cbc_mode)                            // 0 - ECB mode; 1 - CBC mode
{
    ...
    if (ctrl_mode) // ap_ctrl_chain running mode
    {
        ...
        for (k = 0; k < groups_num / thread_num; k++)
        {
            ...
            {
                t[i] = std::thread(execute_cipher_thread, run_krnl_cbc[i], input_sub_buffer[k * thread_num + i], output_sub_buffer[k * thread_num + i], //iv, 
                             words_num, encryption_mode, cbc_mode);
            }
            for (i = 0; i < thread_num; i++)
            {
                t[i].join();
    ...
    else // emulate ap_ctrl_hs running mode
    {
        for (int i = 0; i < groups_num; i++) {
            execute_cipher_thread(run_krnl_cbc[0], input_sub_buffer[i], output_sub_buffer[i], words_num, encryption_mode, cbc_mode);
        }
    }
...
// --------------------------------------------------------------------------------------
    char *plain_data, *added_data;
    plain_data  = new char [words_num * 64 * groups_num];
    //cipher_ecb_data  = new char [words_num * 16 * groups_num];
    //cipher_cbc_data  = new char [words_num * 16 * groups_num];
    added_data = new char [words_num * 64 * groups_num];
    char *output_data = new char [words_num * 64 * groups_num];
    ...
    for (int i = 0; i < words_num * 64 * groups_num; i++) {
    ...
    add_one_data((unsigned char*) plain_data, (unsigned char*)added_data, words_num, groups_num, 1);
    // write out reference data for reference
    write_bin_file("./plain.dat", words_num * 64 * groups_num, plain_data);
    write_bin_file("./added.dat", words_num * 64 * groups_num, added_data);
...
// --------------------------------------------------------------------------------------
    ...
    xrt::bo krnl_cbc_input_buffer = xrt::bo (device, words_num*64*groups_num, xrt::bo::flags::normal, bank_grp_krnl_cbc_read);
    xrt::bo krnl_cbc_output_buffer = xrt::bo (device, words_num*64*groups_num, xrt::bo::flags::normal, bank_grp_krnl_cbc_write);
    
    // create sub-buffer objects
    std::vector<xrt::bo> krnl_cbc_input_sub_buffer;
    std::vector<xrt::bo> krnl_cbc_output_sub_buffer;
    for (int i = 0; i < groups_num; i++) {
        krnl_cbc_input_sub_buffer.push_back(xrt::bo(krnl_cbc_input_buffer, words_num * 64, words_num * 64 * i));
        krnl_cbc_output_sub_buffer.push_back(xrt::bo(krnl_cbc_output_buffer, words_num * 64, words_num * 64 * i));
    }
...
// --------------------------------------------------------------------------------------
    ...
    std::cout << "Throughput = " << words_num * groups_num * 64 / total_run_time * 1000 / (1024 * 1024) << " MB/s" << std::endl << std::endl;


// --------------------------------------------------------------------------------------
// ECB Mode Decryption test
// --------------------------------------------------------------------------------------
    std::cout << std::endl << "-------------- pass through test " << std::endl;
    ...
    // Data validation
    fail_count = data_compare((char*)output_data, plain_data, words_num * 64 * groups_num);
    if (fail_count != 0) {
        std::cout << "Data validation FAILED, error num = " << fail_count << std::endl;
    } else {
        std::cout << "Data validation SUCCESS" << std::endl;
