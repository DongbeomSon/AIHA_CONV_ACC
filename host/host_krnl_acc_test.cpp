...
#define krnl_acc_arg_TILE_NUM 5
#define krnl_acc_arg_IFM_ADDR_BASE 6
#define krnl_acc_arg_WGT_ADDR_BASE 7
#define krnl_acc_arg_OFM_ADDR_BASE 8
...
{
    std::cout << std::endl
              << "    Usage: host_krnl_cbc_test [-g GROUP_NUM] [-w WORD_NUM] [-k KEY] [-v IV] [-v LOOP_NUM] [-s] [-h]" << std::endl;
...
void conv2d_thread(xrt::run run_krnl_acc, xrt::bo ifm_buffer, xrt::bo wgt_buffer, xrt::bo ofm_buffer, int ci, int co, int ifm_size, int wgt_size, int ofm_size, int tile_num)
{
    ...
    run_krnl_acc.set_arg(krnl_acc_arg_TILE_NUM, tile_num);
...
float conv2d(xrt::kernel kernel,                  // kernel handle
             ...
             int wgt_size,
             int ofm_size,
             int tile_num) // output channel = 8 * (co + 1)
{
        ...
        for (k = 0; k < groups_num / thread_num; k++)
        {
            for (i = 0; i < thread_num; i++)
            {
                t[i] = std::thread(conv2d_thread, run_krnl_acc[i], ifm_sub_buffer[k * thread_num + i], wgt_sub_buffer[k * thread_num + i], ofm_sub_buffer[k * thread_num + i],
                                   ci, co, ifm_size, wgt_size, ofm_size, tile_num);
            }
            for (i = 0; i < thread_num; i++)
            {
                t[i].join();
    ...
    else // emulate ap_ctrl_hs running mode
    {
        ...
        {
            conv2d_thread(run_krnl_acc[0], ifm_sub_buffer[i], wgt_sub_buffer[i], ofm_sub_buffer[i], ci, co, ifm_size, wgt_size, ofm_size, tile_num);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    // --------------------------------------------------------------------------------------
    std::cout << std::endl
              << "-------------- Convolution acceleration test -------------- " << std::endl;
    ...
    total_run_time = conv2d(kernel_krnl_acc, ifm_sub_buffer, wgt_sub_buffer, ofm_sub_buffer,
                            groups_num, chain, cfg_ci, cfg_co, ifm_len, wgt_len, ofm_len, tile_num);
